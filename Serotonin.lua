		...
		Library.ESPPreview = function(self, Data)
			...
			local Items = { } do 
				Items["ESPPreview"] = Instances:Create("Frame", {
					...
					BorderSizePixel = 2,
					BackgroundColor3 = FromRGB(17, 21, 27)
				})  Items["ESPPreview"]:AddToTheme({ BackgroundColor3 = "Background 1" })
				...
				Items["Holder"] = Instances:Create("Frame", {
					...
					AnchorPoint = Vector2New(0.5, 0.5),
					BackgroundTransparency = 1,
					Position = UDim2New(0.510869562625885, 0, 0.45294201374053955, 0),
				...
				Items["HealthbarText"] = Instances:Create("TextLabel", {
					Parent = Items["HealthbarAccent"].Instance,
					Name = "\0",
					FontFace = Library.Font,
            ...
            Items["CharacterViewport"]:Connect("InputChanged", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    Yaw = Yaw - math.rad(RotationDelta.X * Sensitivity)
                    Pitch = ClampPitch(Pitch - math.rad(RotationDelta.Y * Sensitivity))

                    LastPosition = Input.Position

                    if ViewportModel and ViewportModel.PrimaryPart then
                        local Rotation = CFrame.Angles(0, Yaw, Roll)
                        ViewportModel:SetPrimaryPartCFrame(Rotation + Vector3.new(0, 2, 0))
                    end
                end
            end)
			...
			local Math = { } do -- esp preview math is done differently
                ...
                function Math:GetBoundingBox(model, camera, ViewportFrame)
                    ...
                    local Position = Vector2New(Min.X + 0.2, Min.Y + 0.45)
                    local Size = Vector2New(Size2D.X + 0.3, Size2D.Y)
			...
			Library:Connect(RunService.RenderStepped, function()	
                local Pos, Size = Math:GetBoundingBox(Preview.Player, ViewportCamera, Items.CharacterViewport.Instance)
                Items.Holder.Instance.Position = Pos
                Items.Holder.Instance.Size = Size
				Items.Healthbar.Instance.Size = UDim2New(0, 1, 0, Items.BoxHolder.Instance.Size.Y.Offset + 4)
            end)
...
local ESPPreview = Library:ESPPreview({Title = "Preview"})
...
ESPSection:Toggle({Name = "Enable ESP", Flag = "ESP Enabled", Callback = function()
	...
	ESPPreview:Set("Name", Library.Flags["ESP Enabled"], {Color = Library.Flags["Names Accent"].Color, Enabled = Library.Flags["Enable Names"], Text = Library.Flags["Display Name"] and game.Players.LocalPlayer.DisplayName or game.Players.LocalPlayer.Name})
...
ESPSection:Toggle({Name = "Boxes", Flag = "Enable Boxes", Callback = function(Value)
	ESPPreview:Set("Box", Library.Flags["ESP Enabled"], {Filled = Library.Flags["Boxes Filled"], FillTransparency = Library.Flags["Boxes Filled Opacity"] / 100, Color = Library.Flags["Boxes Accent"].Color, Enabled = Library.Flags["Enable Boxes"], FillColor = Library.Flags["Filled Boxes Accent"].Color})
...
ESPSection:Toggle({Name = "Chams", Flag = "Enable Chams"}):Colorpicker({Name = "Chams Color", Flag = "Chams Accent", Default = Color3.fromRGB(255, 255, 255)})
ESPSection:Toggle({Name = "Viewangle", Flag = "Enable Viewangle"}):Colorpicker({Name = "Viewangle Color", Flag = "Viewangle Accent", Default = Color3.fromRGB(255, 255, 255)})
ESPSection:Toggle({Name = "Highlight", Flag = "Highlight ESP"}):Colorpicker({Name = "Highlight Color", Flag = "Highlight Accent", Default = Color3.fromRGB(255, 0, 0)})
ESPSection:Toggle({Name = "Team Check", Flag = "Enable TeamCheck"})
ESPSection:Toggle({Name = "Skeletons", Flag = "Enable Skeletons", Callback = function()
	ESPPreview:Set("Skeleton", Library.Flags["ESP Enabled"], {Enabled = Library.Flags["Enable Skeletons"]})
	ESPPreview:UpdateSkeleton(Library.Flags["Skeletons Accent"].Color, Library.Flags["Outline Skeletons/ViewAngle"])
end}):Colorpicker({Name = "Skeletons Color", Flag = "Skeletons Accent", Default = Color3.fromRGB(255, 255, 255), Callback = function()
	ESPPreview:UpdateSkeleton(Library.Flags["Skeletons Accent"].Color, Library.Flags["Outline Skeletons/ViewAngle"])
end})
...
ESPSettings:Toggle({Name = "Outline", Flag = "Outline Skeletons/ViewAngle"})
ESPSettings:Slider({Name = "Max Distance Amount", Flag = "Max ESP Distance", Min = 0, Max = 10000, Default = 1000})
ESPSettings:Slider({Name = "Skeleton Thickness Amount", Flag = "Skeleton Thickness", Min = 1, Max = 10, Default = 1})
ESPSettings:Slider({Name = "Viewangle Length Amount", Flag = "Viewangle Length", Min = 0, Max = 20, Default = 5})
ESPSettings:Slider({Name = "Boxes Filled Opacity Amount", Flag = "Boxes Filled Opacity", Min = 0, Max = 100, Default = 50, Callback = function()
	ESPPreview:Set("Box", Library.Flags["ESP Enabled"], {Filled = Library.Flags["Boxes Filled"], FillTransparency = Library.Flags["Boxes Filled Opacity"] / 100, Color = Library.Flags["Boxes Accent"].Color, Enabled = Library.Flags["Enable Boxes"], FillColor = Library.Flags["Filled Boxes Accent"].Color})
end})
ESPSettings:Slider({Name = "Chams Opacity Amount", Flag = "Chams Opacity", Min = 0, Max = 100, Default = 50})
ESPSettings:Dropdown({Name = "Chams Visible Check", Flag = "Chams Visible Check", Items = {"Always Visible", "Visible", "Invisible"}, Default = "Visible"})
ESPSettings:Dropdown({Name = "Chams Mode", Flag = "Chams Visible Check", Items = {"Always Visible","Visible When Occluded"}, Default = "Visible When Occluded"})
